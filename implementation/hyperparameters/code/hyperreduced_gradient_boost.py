...
file_out = "./grid_search_results_gb.csv"
fdw = open(file_out, "w")
...
def train_and_test_model(X_train, y_train, X_test, y_test):
    ...
    os.system("rm ./results_gb.csv")
    os.system("touch ./results_gb.csv")

    for estimators in range(10, 110, 10):
        for depth in range(10, 60, 10):
            model = GradientBoostingClassifier(n_estimators = estimators, max_depth = depth, random_state = rng)
            ...
            command = 'echo "' + str(to_write) + '" >> results_gb.csv'
            os.system(command)
