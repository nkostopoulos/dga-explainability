...
file_out = "./grid_search_results_extra.csv"
fdw = open(file_out, "w")
...
def train_and_test_model(X_train, y_train, X_test, y_test):
    ...
    os.system("rm ./results_extra_trees.csv")
    os.system("touch ./results_extra_trees.csv")

    for estimators in range(10, 500, 10):
    	model = ExtraTreesClassifier(n_estimators = estimators, n_jobs = -1, random_state = rng)
    	...
    	command = 'echo "' + str(to_write) + '" >> results_extra_trees.csv'
    	os.system(command)
