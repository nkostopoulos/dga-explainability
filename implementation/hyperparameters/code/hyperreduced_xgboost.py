...
file_out = "./grid_search_results_xgboost.csv"
fdw = open(file_out, "w")
...
def train_and_test_model(X_train, y_train, X_test, y_test):
    ...
    os.system("rm ./results_xgb.csv")
    os.system("touch ./results_xgb.csv")

    for estimators in range(10, 210, 10):
        for depth in range(10, 210, 10):
            model = xgb.XGBClassifier(n_estimators = estimators, max_depth = depth, n_jobs = -1, random_state = rng)
            ...
            command = 'echo "' + str(to_write) + '" >> results_xgb.csv'
            os.system(command)
            #fdw.write(to_write + "\n")
